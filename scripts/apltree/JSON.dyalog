...
⍝ Neither references to functions or namespaces nor instances can be serialized.
⍝ Assigned functions are serialized and restored properly.
...
⍝   ---------------------------------------------------------------------
    ...
    ∇ r←{verbose}APLtoJSON a
      :Access public shared
      verbose←{(0<⎕NC ⍵):⍎⍵ ⋄ 1}'verbose'
      r←fromAPL a
    ∇
...
⍝   ---------------------------------------------------------------------
    ...
    ∇ r←NSobj a;n
      ...
      :Else
          r←''
          :For n :In ' '~⍨¨↓a.⎕NL 2 9.1  ⍝ DON'T USE -2 9.1 here!
              :If ~IsScript a.⍎n
                  r,←'"',n,'":',(NSval a.⍎n),','
              :EndIf
          :EndFor
...
⍝   ---------------------------------------------------------------------
    ...
    ∇ r←APLObject a;n
      ...
      :For n :In ' '~¨⍨↓a.⎕NL 9.1
          :If IsScript a.⍎n
              :If verbose
                  ⎕←'Namespace script "',n,'" in ',(⍕a),' ignored'
              :EndIf
          :Else
              r,←(JAchars n),':'    ⍝ the name
              r,←(fromAPL a⍎n),','  ⍝ the value
    ...
    ⍝ json: character vector of JSON
      ...
      :If 0<⍴tokentypes
          ...
          :If arrbeg=1⊃tokentypes
              r←2⊃''parseArray 1
          :ElseIf objbeg=1⊃tokentypes
              r←2⊃parseObject 1
          :EndIf
      :EndIf
      :If 1=⍴,tokentypes
      :AndIf tokentypes∊true false nul string number
          r←2⊃''parseValue 1
      :ElseIf format≡'NS'
          r←APLtoNS r
      :EndIf
...
⍝   ---------------------------------------------------------------------
    ...
    ∇ {r}←ns NSvalue obj;name;value;arr;v;aplName;childNS
      :If obj is array
          r←NSarray obj
      :Else
...
⍝   ---------------------------------------------------------------------
    ...
    ∇ list←parseArr s;txt;level;split
     ...
     ⍝ Find the separating commas
      ...
      level←(+\txt<s∊'[{')-+\txt<s∊']}'
      split←1,(s=',')∧level=0
      split∧←~0,txt ⍝ 17/04/2012 gil : don't split text strings
