...
⍝ * Start an application and return its standard output.
⍝ The two goals are quite different and therefore handled by two _
...
⍝ When you need the actual output of another application then you _
...
⍝ Thanks to Peter Michael Hager (mailto:Hager@Dortmund.net) without whom _
⍝ this class would hardly exist.
⍝ Author: Kai Jaeger ⋄ APL Team Ltd ⋄ http://aplteam.com
⍝ Project home page: http://aplwiki.com/Execute
    ...
    ∇ r←Version
      ...
      r←(Last⍕⎕THIS)'1.5.0' '2015-01-09'
    ⍝ 1.5.0: * `Version` is now returning just the name (no path).
    ...
    ⍝ The left argument is optional. If passed this must be a command space. _
    ⍝ You can call the public shared method "DefaultParms" in order to get _
    ⍝ a command space pre-populated with default settings. Then make _
    ⍝ appropriate changes and pass it as left argument.
    ⍝ == Explicit Result
    ⍝ There are four items returned as explicit result:
    ⍝ # return code with 0 being okay
    ⍝ # processInfo: an integer vector with process related information
    ⍝ # result: that's what is returned by "[]OFF n" where "n" gets the result, _
    ⍝   or "Exit n" in a batch program. When wait is 0 there is no result _
    ...
    ⍝ * Vector of char vectors. All items but the first are taken is input then.
    ...
    ⍝ [0] the return code, 0 is okay. Note that is is NOT the exit code of the application!
    ⍝ [1] the task's result, converted to a vector of vectors.
    ...
    ∇ r←DefaultParms
      :Access Public Shared
      ⍝ Returns a command space with all the parameters and their default settings _
      ⍝ you may specify when calling `Application`.
      ⍝ Note that the namespace provides a method `List` which you can use in _
      ⍝ order to list all names and their current values.
      ⍝ == The parameters
      ⍝ === dir
      ⍝ By default the process is running in the current directory. By _
      ⍝ specifying a different directory you can change this.
      ⍝ === wait
      ⍝ Defaults to 1 which means that `Application` waits until program _
      ⍝ finishes. Note the the "result" part returned by `Application` is meaningless _
      ⍝ if "wait" is 0: naturally you cannot expect a result in such a case.
      ⍝ === hidden
      ⍝ Defaults to 0. That means that command.com will show a console window - _
      ⍝ just an example. If Dyalog is fired up and the app writes to the _
      ⍝ session you will see the session window.
      ⍝ However, if you set this to 1, the session window won't turn up _
      ⍝ no matter whether the app does write to it or not.
      ⍝ Note the danger: if APL crashs and no error trapping is used, then _
      ⍝ the session manager should pop up. If that is suppressed but setting _
      ⍝ "hidden" to 1 then the user has no means to quit that APL task. The only _
