:Class WinSys
⍝ This class makes a variety of methods available which are somehow related _
⍝ to the Windows Operating System.
⍝ In particular it features the `GetSystemMetric` function as well as all the _
⍝ the constants this function is able to deal with.
⍝ It also offers the method `GetMsgFromErrCode` which takes a Windows _
...
⍝ Homepage: http://aplwiki.com/WinSys
    ...
    ∇ r←Version
      ...
      r←({⍵↑⍨-'.'⍳⍨⌽⍵}⍕⎕THIS)'2.3.0' '2016-01-22'
      ⍝ 2.3.0: New methods `SetWindowParms` and `FindWindow`, although the latter is the same as `GetHandleFromCaption`.
      ⍝ 2.2.0: New methods `ShellExecute` and `CreateParms_ShellExecute`
    ∇
    ...
    ∇ {r}←{adminFlag}ShellExecute x;ShellOpen;parms
      ...
      ⍝ "x" can be one of:
      ⍝ * A namespace, typically created by calling `CreateParms_ShellExecute`. This is called a _
      ⍝   parameter space.
      ⍝ * A text string typically specifying a document or an EXE, possibly with command line parameters.
     
      ⍝ A parameter space is usually created by calling `CreateParms_ShellExecute`. You can then make _
      ⍝ amendments to it and pass it as right argument. See there for details.
     
      ⍝ If the defaults are fine for you and you want just start an EXE or, say, display an _
      ⍝ HTML file then you can just specify a path, either to the EXE or to the document.
      ⍝ You can even specify command line parameters this way but you must enclose `file` with _
      ⍝ double quotes (") if it contains blanks. (The `ShellExecute` Windows function does not like _
      ⍝ double quotes but we remove them before we call it).
     
      ⍝ The optional left argument defaults to 0 which makes the verb default to "OPEN". By specifying _
      ...
      :If (⎕DR x)∊80 82
          ...
          parms.file←x
          parms.(file lpParms)←{l←1+⌈/(1↓⍵)⍳' "' ⋄ ('"'~⍨l↑⍵)(1↓l↓⍵)}parms.file
      :ElseIf 326=⎕DR x
      :AndIf 9=⎕NC'x'
          parms←x
    ...
    ∇ R←CONSTANT Y;name;value;tf;cf
      :Access Public Shared
    ⍝ Returns a string. Executing this string creates a niladic function with _
    ⍝ the name of the "Constant". Calling this function returns the desired result. _
    ⍝ The function created this way acts in many respects like a "Constant" in _
    ⍝ other programming languages.
    ⍝ In case Y is empty R is empty, too.
    ⍝ Example:  (creates a function "PI" that returns 3.14)
    ⍝ <pre>⍎CONSTANT 'PI←3.14'</pre>
      :If 0∊⍴Y
          R←''
      :Else
          (name value)←Y{(⍵↑⍺)(1↓⍵↓⍺)}Y⍳'←'
   ...
   ⍝ r[4] = Service pack information or empty
      ...
      '∆GetVersion'⎕NA'I KERNEL32|GetVersionEx',QnaType,' ={I4 I4 I4 I4 I4 T[',(⍕multiByte×128),']}'
    ...
    ⍝ In case of an error r ←→ (¯1 ¯1 ¯1)
      'GetDiskFreeSpaceEx'⎕NA'I KERNEL32|GetDiskFreeSpaceEx',QnaType,' <0T >U[2] >U[2] >U[2]'
    ...
    ⍝ In case of an error R is empty.
      :Access Public Shared
      '∆GetComputerName'⎕NA'I4 KERNEL32.C32|GetComputerName',QnaType,' >0T =P'
      multiByte←1+80=⎕DR' '           ⍝ Unicode version? (used to double the buffer size)
    ...
    ⍝ <pre>#.WinSys.(GetSystemMetrics SM_CYMENU)</pre>
    ...
    ⍝ Returns the directory name of the currently running Windows.
      '∆GetWindowsDirectory'⎕NA'I4 KERNEL32.C32|GetWindowsDirectory',QnaType,' >0T =I4'
      multiByte←1+80=⎕DR' '           ⍝ Unicode version? (used to double the buffer size)
      :Trap 0
    ...
    ∇ r←GetDPI;GetDC;GetDeviceCaps;hdc;sy;ReleaseDC
      :Access Public Shared
    ⍝ Returns the current setting of DPI. Typical settings are 100 (default), 125, 150. _
    ...
    ⍝ Note that you cannot fake a filename in order to find out what program is _
    ⍝ associated with a particular extension. The contents of the file, however, _
    ...
    ⍝ rc: 0 in case of success, otherwise one of:
      ...
      '∆FindExecutable'⎕NA'I4 shell32.C32|FindExecutable',QnaType,' <0T <0T >0T'
      multiByte←1+80=⎕DR' '           ⍝ Unicode version? (used to double the buffer size)
    ...
    ∇ R←List_SM
      :Access Public Shared
    ⍝ Lists all fields with names that start with "SM_"
      R←⊃⎕NL-2
      R←(R[;⍳3]∧.='SM_')⌿R
      R←(↓R)~¨' '
    ∇
    ...
    ⍝ Translate Message ID (mid) to something more useful for human beings.
      ...
      LangID←0
      'FormatMsg'⎕NA'I KERNEL32|FormatMessage',QnaType,' I4 I4 I4 I4 >T[] I4 I4'
      :If 0>mid←↑mid
      :AndIf ¯16777216≤mid
          mid←-mid
      :EndIf
      ...
      :If 0∊⍴r
          'LoadLibrary'⎕NA'I KERNEL32|LoadLibrary',QnaType,' <0T'
  ...
  ⍝ Returns the "SystemParametersInfo" structure. See http://msdn.microsoft.com/en-us/library/ms724947.aspx for details.
    ...
    ∇ h←FindWindow caption;FindWindow
    ⍝ Return handle to window with "caption".
    ⍝ Note that "caption" must match the caption of the *window* exactly although it is *not* case sensitive.
      :Access Public Shared
      'FindWindow'⎕NA'U User32.C32|FindWindow',QnaType,' P <0T'
      h←FindWindow 0 caption
    ∇
    ...
    ∇ filename←GetModuleFileName dllName;dll;length;buffer;∆GetModuleFileName;∆LoadLibrary
      ...
      ⍝ to be smart and not only checks several directories (not only the working dir
      ⍝ as you might think!) but also watches out for DLLs with similar (!) names.
      filename←''
      '∆LoadLibrary'⎕NA'U kernel32|LoadLibrary* <0T'
      dll←∆LoadLibrary⊂dllName
      '∆GetModuleFileName'⎕NA'u kernel32|GetModuleFileName* U >0T u'
    ...
    ⍝ Examples:
    ⍝ WinSys.SetWindowParms hwnd (300 400)  ⍝ (Re-)size the window but does not affect the position.
    ⍝ (5 6) WinSys.SetWindowParms hwnd (300 400)  ⍝ (Re-)size the window and position it at (5 6).
    ...
    ⍝ R is 1 if the process got killed, otherwise 0.
      :Access Public Shared
      r←0
      ⎕NA'U4 KERNEL32.C32|OpenProcess I4 I2 I4'
