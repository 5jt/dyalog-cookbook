:Class HandleError
⍝ This class offers the method `Process` which is useful to handle application _
⍝ errors on a general level. `Process` in turn is triggered via `⎕TRAP`; use _
⍝ the method `SetTrap` in order to set `⎕TRAP` properly.

⍝ Goals:
⍝ * Create a namespace `crash` and populate it with variables providing all _
⍝   sorts of information potentially important for analyzing the error. _
⍝   The namespace "crash" is saved in a component file by default. By default _
⍝   all variables visible at the time of the crash are saved in a sub namespace _
⍝    of "crash" called "Vars".
⍝ * Create an HTML page with essential information regarding the error.
⍝ * Attempt to save an error workspace. In order to achieve that, all _
⍝   running threads are killed except the main thread and, if different, the _
⍝   thread running `Process`.

⍝ All options can be changed by setting variables in the namespace _
⍝ returned by `CreateParms` which can be passed as right argument to the main _
...
⍝ * `logFunction` is supposed to be the name of a monadic function that _
⍝   returns a shy result. That function must be able to deal with a simple _
⍝   string as well as vectors of strings.
⍝ * `customFns` can be used to do anything you like. The right argument is _
⍝   the namespace with the configuration settings, as returned by `CreateParms`. _
⍝   A typical application for this is sending an email to notify certain people _
⍝   about the crash.
⍝ Although both function names may use dotted syntax that would not help in case _
⍝ they live in an instance of a class. In that case use `logFunctionParent` _
⍝ and `customFnsParent` respectively in order to define the parent of the functions.

⍝ How to:
⍝ # At an early stage (we don't know where to save anything etc) specify an _
⍝   empty vector as right argument to `SetTrap`.
⍝ # As soon as you can create a parameter space by calling `CreateParms`, _
⍝   specify the details in it, typically by `⎕SHADOW`ing the name of that _
⍝   parameter space in the root within your `⎕LX` function and then assiging _
⍝   it to something like `#.MyParms`.
⍝   `Process` is then able to see this when you specify
⍝   `⎕TRAP←0 'E' '#.HandleError.Process ''#.MyParms''`

⍝ Note: The attempt to save an error WS will fail if there are any open acre _
⍝ projects. In a production environment that should not be the case normally. _
⍝ If you do not share this opinion then use your own function (see the _
...
⍝ Homepage: http://aplwiki.com/HandleError
    ...
    ∇ r←Version
      ...
      r←(Last⍕⎕THIS)'1.7.3' '2015-11-05'
      ⍝ 1.7.3: * Under certain circumstances `SaveVisibleVars` crashed.
      ⍝ 1.7.2: * `⎕WA` was not reported. Much needed to identify configuration problems.
      ⍝ 1.7.1: * Several issues in the documentation fixed.
      ⍝ 1.7.0: * "CurrentDir" added to the HTML report.
      ⍝ 1.6.0: * The "crash" namespace now also contains a variable "CurrentDir".
      ⍝ 1.5.0: * The "crash" namespace now contains a sub namespace "Vars" in which
      ⍝          all vars visible at the time of the crash are saved.
      ⍝        * `⎕LX` is now set to `⎕TRAP←0 'S' ⍝,⎕LX`.
      ⍝ 1.4.3: * Documentation improved
      ⍝        * Type check on the name of the parameter space was too restrictive.
      ⍝ 1.4.2: Documentation improved
      ⍝ 1.4.1: An empty `errorFolder` did not result in files being created
      ⍝        in the current dir but the top-most dir off the current drive.
    ∇
    ...
    ∇ r←{force}SetTrap parameterSpaceName;⎕TRAP;⎕ML;⎕IO;calledFrom
      :Access Public Shared
    ⍝ Returns a vector useful to set `⎕TRAP` depending on whether it's _
    ⍝ a development session or not.
    ⍝ The right argument can be either an empty vector or the name of _
    ⍝ a parameter space - ''not'' a reference!
    ⍝ The left argument defaults to 0. Setting this to 1 enforces error _
    ...
    ⍝|| `customFnsParent` || No default. May be a reference pointing to the parent of the `customFns`. Needed only in case that is  class instance since `customFns` may use dottet syntax.
    ...
    ⍝|| `logFnsParent` || No default. May be a reference pointing to the parent of the `logFunction`. Needed only in case that is  class instance since `logFunction` may use dottet syntax.
    ...
    ⍝|| `saveVars`   || Boolean that defaults to 1. Is ignored when `saveCrash` is 0. If 1 all visible variables are saved in a sub namepsace `Vars` within `crash`.||
    ⍝|| `signal`     || When "off" is 0 and "signal" is not 0 then "signal" is signalled by "Process". Can be used for a restart attempt.||
    ⍝|| `trapInternalErrors` || By default all internal errors are trapped: `Process` should never crash an application.||
    ⍝|| `trapSaveWSID`       || Boolean that defaults to 1. Makes sure that the ⎕SAVE statement is guarded. Useful to trace through it without a problem in versions before 14.1.||
...
⍝⍝⍝⍝⍝⍝⍝⍝⍝⍝⍝⍝⍝⍝⍝⍝⍝⍝⍝⍝⍝ Private stuff
    ...
    ∇ crash←CreateCrash(parms Trap)
      ...
      crash.WA←⎕WA
      crash.Trap←Trap
      crash.CurrentDir←⊃⎕CMD'cd'    ⍝ We don't know whether WinFile is around
      :Trap (parms.trapInternalErrors)/0
