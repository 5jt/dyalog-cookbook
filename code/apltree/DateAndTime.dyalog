...
⍝ Note that virtually all functions have been implemented by Phil Last _
⍝ (http://aplwiki.com/PhilLast) - unless stated otherwise - and generously _
...
⍝ Homepage: http://aplwiki.com/DateAndTime
    ...
    ⍝ Seer also `Timestamp2DayDecimal`.
    ⍝ Phil Last
      :Access Public Shared
      r←{
          dhm←24 60 60 1000
    ...
    ⍝ `⍵` can be either a vector with 3 elements (Y M D) or a matrix with _
    ...
    ⍝ `⍵` date (like `3↑⎕ts`). Can be either a vector with 3 elements (Y M D) or a matrix with _
    ⍝ n rows and 3 columns.
