...
⍝ *** Version 4.0.0 ⋄ 2016-08-23 ***
⍝
⍝ ## Overview
⍝
⍝ This is a major change in two respects:
⍝
⍝ ### Clean-up
⍝
...
⍝ * `First`
⍝
⍝    Note that there is a function `FormatDateTime` available in the `DateAndTime` class.
⍝
⍝ * `Mix`
...
⍝ Homepage: <http://aplwiki.com/APLTreeUtils>
    ...
    ⍝   "newline" definitions ((⎕UCS 13 10) or ⎕UCS 10 or ⎕UCS 13) is found in the file.
    ⍝
    ...
    ⍝ * A filename. Then the file is tied, read and untied behind the scenes.
    ...
    ∇ {r}←{append}WriteUtf8File(fileIdentifier data);filename;fno;fullname;flag;⎕ML;⎕IO;i;max;size;simpleFlag;wasOpenFlag;newline
     ⍝ Note that Dyalog 15.0 come with the newly introduced system functions ⎕NPUT
     ⍝ and ⎕NGET. They offer a powerful, flexible and platform-independent interface
     ⍝ for reading and writing and files.\\
     ⍝ However, `ReadUtf8File` and `WriteUtf8File` remain available in `APLTreeUtils`
     ⍝ for two reasons:
     ⍝ * Compatability
     ⍝ * Dealing with slippery network. `WriteUtf8File` does not easily give up!
     ⍝ * `WriteUtf8File` allow you to append data **to the last record** rather than
     ⍝   appending records if you wish so.
     ⍝
     ⍝   This is not POSIC compatible yet most software (~70%) available offer this
     ⍝   feature, and their are application for this!
     ⍝
     ...
     ⍝
     ⍝ Under Windows `newline←⎕UCS 13 10` while `newline←⎕UCS 10` otherwise.
     ⍝ When a nested vector is passed as data then "newline" is appended to every single
     ⍝ record **but** the last one.\\
     ⍝ That means that there will be no "newline" at the end of the file. That violates the
     ...
     ⍝ this in any case.\\
     ⍝ It means you are in charge for inserting the correct "newline" characters (if any)
    ...
    ⍝ Fires up the default browser and goes to "Url". Exception is when the current
    ...
    ⍝ as the left argument. That will then be used to show "Url". Nore that the left
