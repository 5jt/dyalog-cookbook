...
> `⍝TOTO⍝` This chapter cannot be finished before the problem with saving variables I found in the `]snap` command gets fixed.
>
...
While CHM files are Windows specific, `Markdown2Help` allows you to export the help system as a web page that can be displayed with any modern browser. That makes it OS-independent. We'll discuss later how to do this.
...
`Markdown2Help` is an ordinary (non-scripted) namespace. We therefore need to copy it from its workspace. We also need the script `MarkAPL` which is used to convert the help pages (which are written in Markdown) to HTML. You know by now how to download scripts from the APLTree library. Modify the DYAPP so that our boot function loads the module `MarkAPL` and also copies `Markdown2Help`:
...
* `CreateStub` will check the callback associated with the `Fix` event. If that happens to be a SALT function `CreateStub` will ask whether the new help project should be managed by SALT. 

  If the user confirms this she is given the opportunity to select a drive and/or folder where the help project will be saved. From then on SALT will keep track of the insertion or deletion of nodes and help pages.
...
* The first line specifies a key-value-pair. "index" is the key and "Copyright" is the value of that key.
...
Even if you are familiar whith Markdown you should read `Markdown2Help`'s own help file before you start using Markdown2Help seriously. Some Markdown features are not supported by the help system, and internal links are implemented in a simplified way.
...
Most of the parameters should explain themselves, but if you are in doubt you can always start `Markdown2Help`'s own help system with `#.Markdown2Help.Selfy 0` and read the pages under the "Parameters" node. That's what you would see:
